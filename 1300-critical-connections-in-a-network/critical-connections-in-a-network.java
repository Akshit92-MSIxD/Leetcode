...
class Solution {
    ...
    void dfs(List<List<Integer>> graph, int curr, int parent,
             int[] tin, int[] low, boolean[] vis,
             List<List<Integer>> bridges) {

        vis[curr] = true;
        tin[curr] = low[curr] = timer++;   // ✅ correct timing

        List<Integer> neighbors = graph.get(curr);  // ✅ avoid repeated calls
        ...
        for (int nbr : neighbors) {
            ...
            if (!vis[nbr]) {
                ...
                dfs(graph, nbr, curr, tin, low, vis, bridges);

                low[curr] = Math.min(low[curr], low[nbr]);

                if (low[nbr] > tin[curr]) {
                    // ✅ lighter than List.of + ArrayList
                    bridges.add(Arrays.asList(curr, nbr));
                }

            } else {
                low[curr] = Math.min(low[curr], tin[nbr]);
            }
        }
    }
    ...
    public List<List<Integer>> criticalConnections(int n, List<List<Integer>> connections) {
        ...
        for (int i = 0; i < n; i++) {
            if (!vis[i]) {
                dfs(graph, i, -1, tin, low, vis, bridges);
            }
        }
